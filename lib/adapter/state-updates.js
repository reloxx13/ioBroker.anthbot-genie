...
function chargerPointInMeters(context) {
    const mapMetadata = context?.mapMetadata || context?.mapRaster?.metadata;
    const chargerPoint = mapMetadata?.charger_point;
...
function buildDeviceStateUpdates({
    ...
    const chargerInMeters = chargerPointInMeters(context);
    ...
    const updates = {
        ...
        'location.charger.x': chargerInMeters?.x ?? null,
        'location.charger.y': chargerInMeters?.y ?? null,
