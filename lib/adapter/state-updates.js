 ...
 */
function mowerPoseInMeters(data) {
    const pose = data?.pose && typeof data.pose === 'object' ? data.pose : {};
    const x = typeof pose.x === 'number' ? pose.x / 1000 : null;
    const y = typeof pose.y === 'number' ? pose.y / 1000 : null;
 ...
 */
function chargerPointInMeters(context) {
    const chargerPoint = context?.mapRaster?.metadata?.charger_point;
    const x = typeof chargerPoint?.x === 'number' ? chargerPoint.x / 1000 : null;
    const y = typeof chargerPoint?.y === 'number' ? chargerPoint.y / 1000 : null;
    if (!Number.isFinite(x) || !Number.isFinite(y)) {
        return null;
    }
    return { x, y };
 ...
 * includeMowedPathImage?: boolean,
 * }} params
 * @returns {Record<string, ioBroker.StateValue>}
 */
function buildDeviceStateUpdates({
    context,
    data,
    eventCodeCache,
    ...
    const pose = data?.pose && typeof data.pose === 'object' ? data.pose : {};
    const poseInMeters = mowerPoseInMeters(data);
    const chargerInMeters = includeMapImages ? chargerPointInMeters(context) : null;
    ...
    const updates = {
        ...
        'location.gps.longitude':
            typeof safeGet(data, 'anti_loss_pose', 'posegps', 'lon') === 'number'
                ...
                : null,
        'location.pose.x': poseInMeters?.x ?? null,
        'location.pose.y': poseInMeters?.y ?? null,
    ...
    if (includeMapImages) {
        updates['map.image'] = asText(mapImages.image);
        updates['map.imageWithRtkMask'] = asText(mapImages.imageWithRtkMask);
        updates['location.charger.x'] = chargerInMeters?.x ?? null;
        updates['location.charger.y'] = chargerInMeters?.y ?? null;
        if (includeMowedPathImage) {
            updates['map.imageWithMowedPath'] = asText(mapImages.imageWithMowedPath);
            updates['map.mowedPath'] = asText(mapImages.mowedPath);
        }
    }
    return updates;
}
